...
{
    if (astroke <= 0)
        throw runtime_error("Error: invalid bowstring stroke! Try again!");
...
{
    if (aforce <=0)
        throw runtime_error("Error: invalid pulling force! Try agagin!");
    force=aforce;
}
...
{
    if (aspeed <= 0)
        throw runtime_error("Error: invalid speed! Try again!");
    speed = aspeed;
}
...
void Crossbow::readElFromConsole()
{
    ...
    cin >> t;
    if (t >= 300 || t<=0)
        throw runtime_error("Invalid crossbow range!!! (< 300m)");
    SetRange(t);
    cout << "" + MENU_STRING_TYPE_ET + "";
    while (true)
    {
        cin >> t;
        if (!((t < 1) || (t > TYPE_COUNT_E -1)))
            break;
