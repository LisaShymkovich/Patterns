...
{
    if (aweight <= 0)
        throw runtime_error("Error: invalid weight of gun! Try again!");
    weight = aweight;
}
...
{
    if (acaliber <=0)
        throw runtime_error("Error: invalid caliber! Try again!");
    caliber = acaliber;
}
...
{
    if (acapacity<=0)
        throw runtime_error("Error: invalid magazine capacity! Try again!");
    capacity=acapacity;
}
...
{
    if (aspeed <=0)
        throw runtime_error("Error: invalid bullet speed! Try again!");
    speed=aspeed;
}
...
void Gun::readElFromConsole()
{
    ...
    cin >> t;
    if (t >= 50 || t<=0)
        throw runtime_error("Invalid gun range!!! (< 50m) ");
    SetRange(t);
