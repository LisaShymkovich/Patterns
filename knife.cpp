...
{
    if (alength <= 0)
        throw runtime_error("Error: invalid blade length! Try again!");
    length = alength;
}
...
{
    if (aWeight < 0)
        throw runtime_error("Error: invalid weight of knife! Try again!");
    weight = aWeight;
}
...
void Knife::readElFromConsole()
{
    ...
    cin >> t;
    if (t >= 10 || t <=0)
        throw runtime_error("Invalid knife range! (< 10m)");
    SetRange(t);
    cout << "" + MENU_STRING_Knife_T + "";
    while (true) {
        cin >> e;
        if (!((e < 1) || (e > Knife_COUNT_T -1)))
            break;
